...
\begin{figure}
\centering
\hspace{-0.5cm}\includegraphics[scale=0.6]{M_p.pdf}
...
$k_{\parallel}>XXX$ h/Mpc (dot-dashed lines), corresponding to the approximately white level in Figure~\ref{}.  To match the observed level in Stokes I high band, as shown in required an assumed system temperature of 2400 K, higher than that expected based on the sky and beam model from the simulations, or from the measured calibrated autocorrelations.  The cause of this excess is not understood.
...
%{highband_by_day.pdf}
\caption{High band power as a function of baseline length for $k_{\parallel}=0$ h/Mpc (solid lines) and an average value over the white noise ($k_{\parallel}>1$ h/Mpc; dot-dashed lines) for pseudo-Stokes I on each JD of observation. The black dashed line represents the approximate noise power assuming a receiver temperature of XXX\,K. A very similar relationship obtains for the low band (not shown), but with a higher noise floor (XXX K).  Note that the increase of the noise level with baseline length is correctly modeled, as the compact array has has a decreasing number of baselines which are averaged together at a given $(k_{\parallel},k_{\perp})$ bin.}
