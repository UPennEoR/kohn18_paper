...
\begin{figure*}
\centering
\includegraphics{M_p.png}
%[scale=0.4]{hera19_linear_polarization_beam.pdf}
...
HERA is designed to be calibrated using redundant calibration techniques \citep{DillonParsons16}, but for this preliminary view of HERA commissioning data, we used image-based calibration. Future studies with deeper integrations targeting EoR detections will take advantage of redundancy to obtain more precise calibration solutions \citep{deBoer17}. We used the \casa\ \citep{casa} package for calibration, taking advantage of its CLEAN, {\tt gaincal} and {\tt bandpass} functions.  \edit1{We first converted from native {\sc miriad} to a {\sc uvfits} file format using {\sc pyuvdata} \citep{pyuvdata}; this could then be ingested by \casa.}

\edit1{The calibration sources used in previous PAPER analysis, namely Pictor A \citep{jacobs14} and Fornax A were not available for this observing window (10.5 - 23 h RA), and the few long baselines available made calibration using individual fainter point sources difficult.  We therefore developed a calibration method using the Galactic Center (GC; taken to be at $\alpha, \delta$ = 17h 45m 40.04s, -29d 0m 28.12s) as modeled by the GSM, as implemented in pyGSM \citep{}.  Specifically, we selected four minutes of data centered at the transit of the GC to use for calibration.   The visibilities were phased from drift-scan made to a single phase center chosen to be the LST at the start of the observations.  Because the phase center is not adjusted on-the-fly during the mapping process (that is, because we are imperfectly approximating the tracking telescope assumed by \casa), the length of the observation was chosen to minimize the effects of beam-dependent gain variations as the GC transited.  The calibration was done as a two-step process.  First, we built a CLEAN model with the GC as an unpolarized, flat-spectrum source with an (arbitrary) flux density of 1 Jy.  This allowed us to solve for the large antenna based delay terms ({\tt gaintype='K'}; typically 10's of ns) and a complex bandpass.  With this first calibration in place, the second step was an interactive CLEAN of the GC, still assuming an unpolarized source, but allowing multiple components within a two degree radius centered on the GC.  This was followed by a second round of delay and bandpass calibration to this extended model.  At this point, it remained to determine the overall frequency-dependent amplitude to scale the gains from the arbitrary 1 Jy normalization.  For this we used our simulation of the GC from the GSM, converted to units of Jy, determining a single, spectrally-smooth function to place make the spectrum of the observations match the simulations.  The resulting gains }
%
%The initial goal of the calibration was simply to obtain the large antenna-dependent delay  
%}
...
%\begin{figure*}
%\centering
%\includegraphics[width=0.9\textwidth]{RealAndSim_FluxDensitySpectrum.png}
%\caption{
%The extracted Galactic Center spectrum from images of both the simulated and real data.  showing that spectral structure is due to real features of the sky and beam and not induced by the absolute calibration process.
%}
%\label{fig:GCSpectrum}
%\end{figure*}
...
\caption{Bandpass solutions for the North-South dipole orientation obtained for the functioning antennae in the array on JD 2457548. Differences in line color and style is merely to distinguish different antennae. Shaded regions indicate the effective sub-bands used for power spectrum analysis.  \edit1{The order of these figures changed.}}
\label{fig:bandpass}
\end{figure}
...
Both sets of panels show the Galactic Center (our calibrator source) close to transit in pseudo-Stokes I, Q, U and V visibilities (\textit{top left, top right, lower left, lower right}). A Briggs-weighting with robustness 0 was used when gridding into the image plane. 
	An interactive CLEAN with sources near the Galactic Center was performed.  
