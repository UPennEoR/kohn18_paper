...
Referring to Figure \ref{fig:bl0_cuts_vs_sim_zoom}, \edited{calibrating the raw
  ...
  disagreeing by $\sim$20\%. A potential reason for the disagreement in the low
  band is a less accurate model of the primary beam, which is known to
  systematically underpredict the response to a given sky signal. In the
  analysis that follows, we have increased the amplitude of the simulated power
...
\edited{In the high band analysis, the beam leakage modeled in the simulation is
  ...
  spectra), which can be accounted for by a $\sim$5\% relative amplitude
  miscalibration between the E and N polarized feeds of the antenna. The
  ...
  band \cite{Moore17}, although noise prevents probing levels as deep as those
  in \cite{Asad18}.}
...
\edited{As with pseudo-Q and U, pseudo-V has excess power in the measured power
  ...
  to leak into pseudo-V. However, given the measured amplitudes of the two power
  spectra implies that this systematic error in phase angle must be order
  $\pi/2$, which would imply that these phase errors are nearly maximally
  ...
  to the diagonal elements of the Jones matrix are $\sim$3\%. This is similar to
...
\edited{The low band measurements tell a similar story, with the notable
  ...
  discrepancy than the high band measurements. As with the overall power in
  pseudo-I, this difference may be attributable to the less accurate beam model
  at these frequencies. Performing the same analysis as for the high band, we
