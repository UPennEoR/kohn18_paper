...
\begin{figure*}
\centering
\includegraphics[scale=0.4]{hera19_linear_polarization_beam.pdf}
\caption{\edit1{A different view of the direction dependent Mueller matrix at 120 MHz and 160 MHz (\textit{above} and \textit{below}, respectively).   The left column is $M_{II}$ (upper left corner of Figure \ref{fig:mueller}); the right column has a color scale given by $M_p = \sqrt{M_{IQ}^2 + M_{IU}^2}$ and the marker indicates the orientation of the spin-2 function $M_{IQ} + i M_{IU}$.
}
}
...
\edit1{Need to describe the new calibration scheme}
